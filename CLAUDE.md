...
**Last audit:** 2026-04-09 (`~/projects/home-assistant/ideas/2026-04-09-full-audit.md`)
**Primary backlog:** `~/projects/home-assistant/BACKLOG.md`
...
| **Game Room** | SmartLight-2 (ThirdReality Smart Night Light-W) |
...
Home Assistant
        ...
        │
        ├─► 4 ThirdReality Smart Night Lights (ZigBee via Matter)
        │       light.smartlight_1 ← Theatre (cinema light, goes red 20% when projector on)
        │       light.smartlight_2 ← Game Room (not in the unified automation)
        │       light.smart_night_light_w ← (this is SmartLight-3, Concession Kitchen)
        │       light.smartlight_3 ← logical #3 in automation
        │       light.smartlight_4 ← Office
        │       ** Naming is inconsistent — see §9 Footguns **
        │
...
`.storage/lovelace_dashboards` defines **7 dashboards**, 6 storage-mode + 1 YAML (`lovelace-home`) mapped from `ui-lovelace.yaml`.
...
| `/clamlings` | Clamlings | storage | — | — | **Staged but not yet built-out** — kids dashboard. Waiting for HA restart + MVP card stack. Bedtime gate already wired (see §6 automations). See `~/projects/home-assistant/ideas/2026-04-09-kids-remote-playback.md`. |

**OPEN PROPOSAL — Lovelace cleanup:** `~/projects/home-assistant/ideas/2026-04-09-lovelace-dashboard-cleanup.md` recommends **deleting `ui-lovelace.yaml` entirely** plus the `lovelace:` block in `configuration.yaml`. Rationale: only 3 duplicate cards, HACS resources already in UI registry. **Not yet executed — pending Eddie's approval.** Flag this before any edit to `ui-lovelace.yaml`.
...
Derived from the root CLAUDE.md, the audit, and the BACKLOG:

- **Per-project `BACKLOG.md`** in project root, tagged `MAJOR` / `MINOR` / `PATCH`. Prometheus updates it after every session.
...
- **Strict audit → approval → commit → push protocol:**
  1. Backlog item defined
  2. Prometheus writes code
  3. QC (project-level + VP of QC)
  4. Deploy to live for Eddie's review
  5. Eddie approves the live deploy
  6. Version bump + changelog + commit
  7. Push to GitHub immediately
  8. Container rebuild
  - **Nothing hits GitHub until Eddie has approved the live deploy.**
...
12. **The YAML lovelace dashboard is scheduled for deletion.** Before editing `ui-lovelace.yaml`, check `~/projects/home-assistant/ideas/2026-04-09-lovelace-dashboard-cleanup.md` — the proposal is to delete it entirely along with the `lovelace:` block in `configuration.yaml`.

13. **SmartLight naming is inconsistent.** The "4 basement smart lights" are actually:
    - `light.smartlight_1` (Theatre)
    - `light.smart_night_light_w` (Concession Kitchen — this is the physical "SmartLight-3" in the device registry, but the entity name is `smart_night_light_w`)
    - `light.smartlight_3` (logical slot 3 in the automation)
    - `light.smartlight_4` (Office)
    - Plus `light.smartlight_2` in the Game Room which is **NOT** part of the unified basement automation.
    If you're looking at the automation and wondering where "SmartLight-2" went — it's intentionally excluded.
...
├── BACKLOG.md                       # Source of truth for open work, tagged MAJOR/MINOR/PATCH
├── ideas/
...
└── config/
    ├── CLAUDE.md                    # THIS FILE — reference context for future sessions
...
**End of CLAUDE.md.** If you add major features, integrations, or rooms, update the relevant section and bump the "Last audit" date at the top.
